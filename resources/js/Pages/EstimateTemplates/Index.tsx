...
export default function Index({ auth, templates }: Props) {
    ...
    return (
        ...
        >
                                        ...
                                        {templates.data.map((template) => (
                                            <tr key={template.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                                    <Link
                                                        href={route('estimate-templates.edit', template.id)}
            ...
            <Modal show={!!templateToDelete} onClose={() => setTemplateToDelete(null)}>
                <div className="p-6">
                    <h2 className="text-lg font-medium text-gray-900">
                        テンプレートを削除しますか？
                    </h2>
                    <p className="mt-1 text-sm text-gray-600">
                        「{templateToDelete?.name}」を削除します。この操作は取り消せません。
                    </p>
                    <div className="mt-6 flex justify-end gap-x-3">
                        <SecondaryButton onClick={() => setTemplateToDelete(null)} disabled={processing}>
                            キャンセル
                        </SecondaryButton>
                        <button
                            onClick={handleDelete}
                            disabled={processing}
                            className="inline-flex items-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-sm font-semibold text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150 disabled:opacity-25"
                        >
                            削除する
                        </button>
                    </div>
