...
export default function Show({ auth, invoice, isLatest, hasFinalized }: Props) {
    ...
    const handleRevision = () => {
        if (confirm('新しく訂正バージョンを作成しますか？')) {
            router.post(
                route('invoices.revision', invoice.id),
                {},
                {
                    onStart: () => setProcessing(true),
                    onFinish: () => setProcessing(false),
            ...
            header={
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center sm:gap-0">
                    <h2 className="text-xl font-semibold leading-tight text-gray-800">
                        {getDocType(invoice.status)}詳細:{' '}
                        {invoice.estimate_number} (v{invoice.version})
                    </h2>
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {!['rejected', 'payment_confirmed'].includes(
                            invoice.status
                        ) && (
                            <a
                ...
                <div className="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                        ...
                        <div className="space-y-6">
                            ...
                            {isLatest && (
                                <div className="space-y-6">
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                steps.length -
                                                                    1 ? (
                                                                    <div
                                                                        className={`absolute left-4 top-4 -ml-px mt-0.5 h-full w-0.5 ${isCompleted ? 'bg-indigo-600' : 'bg-gray-200'}`}
                                                                        aria-hidden="true"
                                                                    />
                                                                ) : null}
                                                                <div className="group relative flex items-start">
                                                                    <span className="flex h-9 items-center">
                                                                        ...
                                                                        >
                                                                            ...
                                                                            isRejected ? (
                                                                                ...
                                                                                <span
                                                                                    className={`h-2.5 w-2.5 rounded-full ${isCurrent ? 'bg-indigo-600' : 'bg-transparent'}`}
                                                                                />
                                                                            )}
                                                                        </span>
                                                                    </span>
                                                                    <span className="ml-4 flex min-w-0 flex-col">
                                                                        <span
                                                                            className={`text-sm font-medium ${isRejected ? 'text-red-600' : isCurrent ? 'text-indigo-600' : isCompleted ? 'text-gray-900' : 'text-gray-500'}`}
                                                                        >
                                                                            {isRejected
                                                                                ? '提出済み (失注)'
                                                                                : step.label}
                                    ...
                                    {invoice.status !== 'payment_confirmed' && (
                                        <div className="overflow-hidden border border-indigo-100 bg-white p-6 shadow-sm sm:rounded-lg">
                                            ...
                                            <div className="space-y-3">
                                                {invoice.status ===
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={processing}
                                                        className="inline-flex w-full items-center justify-center rounded-md border border-indigo-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-indigo-700 transition duration-150 ease-in-out hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                                    >
                                                        見積提出済みに戻す
                                                    </button>
                                                )}
                                                {invoice.status ===
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={processing}
                                                        className="inline-flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white transition duration-150 ease-in-out hover:bg-indigo-700 focus:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 active:bg-indigo-900"
                                                    >
                                                        見積を提出する
                                                    </button>
                                                )}
                                                {invoice.status ===
                                                    ...
                                                    <div className="flex flex-col gap-3">
                                                        ...
                                                        <button
                                                            ...
                                                            className="inline-flex w-full items-center justify-center rounded-md border border-red-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-red-700 transition duration-150 ease-in-out hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                                                        >
                                                            失注とする
                                                        </button>
                                                    </div>
                                                ...
                                                {invoice.status ===
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={processing}
                                                        className="inline-flex w-full items-center justify-center rounded-md border border-transparent bg-amber-600 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white transition duration-150 ease-in-out hover:bg-amber-700 focus:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 active:bg-amber-900"
                                                    >
                                                        請求書の作成を開始
                                                    </button>
                                                )}
                            ...
                            <div className="overflow-hidden bg-white p-6 text-sm shadow-sm sm:rounded-lg">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium">
                                            {invoice.issuer_registration_number ||
                                                '-'}
                                        </p>
                                    </div>
                                    <div>
                            ...
                            {/* Danger Zone */}
                            <div className="overflow-hidden border-2 border-red-100 bg-white p-6 shadow-sm sm:rounded-lg">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button
                                        ...
                                        disabled={processing}
                                        className="w-full rounded-md border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-600 shadow-sm transition-colors hover:bg-red-600 hover:text-white"
                                    >
                                        見積を削除する
                                    </button>
                                </div>
