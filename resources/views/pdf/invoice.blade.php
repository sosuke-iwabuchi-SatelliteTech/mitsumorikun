...
@section('content')
    @php
        ...
        // 分割ロジック: 1ページ目は12行、2ページ目以降は15行
        $firstPageItems = array_splice($allDetails, 0, 12);
        $pages[] = $firstPageItems;
        
        if (count($allDetails) > 0) {
            $remainingPages = array_chunk($allDetails, 20);
    ...
    @foreach($pages as $pageIndex => $pageItems)
        @php $isFirstPage = ($pageIndex === 0); $isLastPage = ($pageIndex === $totalPages - 1); @endphp
        ...
        <div class="pdf-page">
            ...
            @else
                <div style="text-align: right; font-size: 10pt; margin-bottom: 5px;">
                    No. INV-{{ $invoice->estimate_number }}-{{ sprintf('%02d', $invoice->version) }} ({{ $pageIndex + 1 }}/{{ $totalPages }})
                </div>
                <div class="title" style="font-size: 16pt; margin-bottom: 10px;"><span>請求書（続）</span></div>
            @endif
            ...
            <table class="details-table">
                ...
                <tbody>
                    ...
                    @php
                        $targetCount = $isFirstPage ? 12 : 20;
                        $fillCount = max(0, $targetCount - count($pageItems));
                    @endphp
